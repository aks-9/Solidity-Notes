...
contract DynamicSvgNft is ERC721 {
    ...
    constructor(
        ...
        // Now when somebody calls the 'tokenURI' function, we'll update the update the images URI in our Metadata based on the price point. For this we need to know the price of the asset, so we'll use Chainlink data feed for that.

        // Intall Chainlink contracts: yarn add --dev @chainlink/contracts
    ...
    function mintNft(
        ...
        s_tokenCounter = s_tokenCounter + 1; //j it's best practice to update the token counter before the minting of NFT to avoid any attack.

        _safeMint(msg.sender, s_tokenCounter);
        // s_tokenCounter = s_tokenCounter + 1;

        emit CreatedNFT(s_tokenCounter, highValue); // evet emitted.
    }
        ...
        returns (string memory)
    {
        ...
        (, int256 price, , , ) = i_priceFeed.latestRoundData(); // Only extracting 'price' from all the values returned from calling the function 'latestRoundData' on our price feed contract.
