//* 22. Intro to 'abi.encode' & 'abi.encodePacked'
...
contract Encoding {
    ...
    function encodeStringBytes() public pure returns (bytes memory) {
        bytes memory someString = bytes('some string'); // this will also give the same result as 'abi.encodePacked' but in the backend they are a little different.
        // https://forum.openzeppelin.com/t/difference-between-abi-encodepacked-string-and-bytes-string/11837
        return someString;
    }

    //* For DECODING
    // using 'abi.decode()', which takes 2 parameters-- an encoded message, and the type of variable we want to decode the message into. Here we're decoding the output of 'encodeString' function, and want the result as a type 'string'.
    ...
    */
    ...
    //*s As of 0.8.13, you can now do `string.concat(string1, string2)`
