...
// Now that we can get a hash for our SVG image from 'svgToImageURI' function, we need to stick it into our JSON metadata. Once that is done, we will encode this new JSON Metadata with base64, and that is going to be our final URI that our token will use.
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    // And now we'll concatenate the '_baseURI' and our Metadata's base64 hash, we'll again use 'abi.encodePacked' and pass '_baseURI() and whatever is returned from 'Base64.encode()'.
                )
